...
- 🎯 Entry points to execute your program using `python -m <sanfoo>` or `$ sanfoo` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/lt8cn/sanfoo/branch/main/graph/badge.svg?token=sanfoo_token_here)](https://codecov.io/gh/lt8cn/sanfoo)
[![CI](https://github.com/lt8cn/sanfoo/actions/workflows/main.yml/badge.svg)](https://github.com/lt8cn/sanfoo/actions/workflows/main.yml)

Awesome sanfoo created by lt8cn

## Install

from source
```bash
git clone https://github.com/lt8cn/sanfoo sanfoo
cd sanfoo
make install
```
...
```bash
$ sanfoo run --port 8080
```

or

```bash
python -m sanfoo run --port 8080
```
...
```bash
❯ sanfoo --help
Usage: sanfoo [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ sanfoo create-user --help
Usage: sanfoo create-user [OPTIONS] USERNAME PASSWORD
...
```bash
sanfoo create-user admin admin --superuser
```
...
Run with `sanfoo run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `sanfoo create-user`.
...
-----------------------------------------------------
sanfoo/__init__.py              4      0   100%
sanfoo/app.py                  16      1    94%
sanfoo/cli.py                  21      0   100%
sanfoo/config.py                5      0   100%
sanfoo/db.py                   10      0   100%
sanfoo/models/__init__.py       0      0   100%
sanfoo/models/content.py       47      1    98%
sanfoo/routes/__init__.py      11      0   100%
sanfoo/routes/content.py       52     25    52%
sanfoo/routes/security.py      15      1    93%
sanfoo/routes/user.py          52     26    50%
sanfoo/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from sanfoo.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in sanfoo/default.toml.

### As environment variables
```bash
export sanfoo_KEY=value
export sanfoo_KEY="@int 42"
export sanfoo_KEY="@jinja {{ this.db.uri }}"
export sanfoo_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
sanfoo_ENV=production sanfoo run
```
